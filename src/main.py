...
def randomkuva():
    ...
    prediction = nv.ennusta([X_testi[i]])[0]
    return {
        "kuva": base64.b64encode(buffered.getvalue()),
        "tunnistus": str(prediction),
        "arvo": int(y_testi[i]),
    }
...
def predict(data: str = Body()):
    ...
    prediction = nv.ennusta([np_image])
    return {"numero": str(prediction[0]), "kuva": base64.b64encode(buffered.getvalue())}
